...
**公開デモ**: https://unicircle-connect-v2.vercel.app
（ログイン画面の「学生として見る／大学職員として見る／高校生・一般として見る」から、登録なしで入れます。
中身はすべて架空のデモデータです）
...
- **サークル** — 設立を申請する、参加を申し込む、メンバーと活動記録を見る、退会する。分野（運動・音楽など）で絞って探せる
- **代替わり** — 代表を次の人に引き継ぐ。相手が受けて初めて成立し、前の代は申し送りを残せる
- **イベント** — サークル・大学のイベントを見て参加登録する。開始の何分前に知らせるかを1件ずつ選べる
- **掲示板** — 所属サークルごとの連絡。貼った紙は2週間で自然に下がり、お知らせに固定したものだけ残る
- **施設予約** — 教室・備品の空きを見て予約を申請する
- **マイページ** — プロフィール、参加履歴、予約履歴、通知設定、表示（明るさと配色）

### 職員（大学の事務）

- **対応待ち** — ログイン後の着地点。自分の承認を待っている設立・廃止の申請と、予約の申請の件数が並ぶ。件数はヘッダーにも1つにまとめて出す（スマホではメニューのボタンに出る）
- **承認** — サークルの設立・廃止、施設予約の申請を承認／却下する。**自分の印影で押し、誰がいつ何を通したかが記録に残る**。承認待ちの1件ごとに押印欄（押された印影と空の欄、「あと○人」）が見え、自分が押したものには「承認済み」と出る
- **申請書** — 承認待ちから申請書を開き、中身を読んだその場で押せる。押すと承認欄の空の枠に印影が入る。承認済みの申請は、印影の並んだ A4 の申請書として印刷できる
- **予約のまとめて承認** — 承認待ちの予約を選んで一度に通す。通らなかった分は理由を出す
- **承認のきまり** — 設立と廃止に何人の承認を求めるかを大学ごとに決める（1〜5人）
- **マスタ管理** — 学生の登録、キャンパス、施設・備品
- **代替わりの確認** — 今年度まだ代表が替わっていないサークルの一覧
- **イベント** — 大学主催のイベントを作る。会場と参加・申込みの方法を書き、学外の方にも案内するかを選べる

### 一般（高校生・企業）／未ログイン

- **サークルを探す** — 都道府県 → 大学 → サークル と辿る。キャンパス単位でも、分野でも探せる。
  連絡先が書かれていないサークルでは、大学の窓口と公式サイトを案内する
- **気になる大学・サークル** — 見る大学を指定しておく、サークルをお気に入りに入れる
- **イベント** — 学外向けに公開されたものだけが並ぶ（オープンキャンパス・学祭など）。
  日時・会場・参加方法を最初に出し、終わったものには「終了」と付ける
- 閲覧が目的なので、**イベントの参加登録と施設予約はできません**
- 一般アカウントは自分で退会できます（学生・職員は大学が管理する情報なので不可）

### みんなに共通

- **明るさ** — 「端末に合わせる／明るい／暗い」を画面の下とマイページで選べる。選択はその端末に残る
- **配色** — 5種類のテーマ。形（角丸・影・ボタン）は共通で、変わるのは色だけ
- **スマホ** — 412px 幅でも横にはみ出さない。小さいボタンも押せる範囲は 44px 四方
...
| `0029_seals.sql` | 印影と、承認記録をハッシュで繋ぐ仕組み |
| `0030_handover.sql` | 代替わり（引き継ぎ）と年度 |
| `0031_events_public_listed_rls.sql` | 学外に出さないイベントをポリシーでも塞ぐ |
| `0032_event_guide.sql` | イベントの会場と、参加・申込みの方法 |
| `0033_circle_category.sql` | サークルの分野 |
| `0034_approval_chain_lock.sql` | 同時に押しても、承認の記録の連なりと人数の判定が崩れないようにする |
...
| 7 | `supabase/seed_review_scenarios.sql` | 承認・廃止・退会の各状態を作った確認用データ（学外の一覧には出さない） |
| 8 | `supabase/fix_demo_data.sql` | 分野・会場と申込み方法・学外向けの紹介文を埋め、時刻のずれを直す |
...
データが載ります。時刻は**日本時間の昼間**に入ります。**どれも何度実行しても増えません。**

`8` は seed が持たない情報（分野・会場など）を補うので、seed を入れ直したあとにも流してください。
最後に確認用の件数が並び、すべて 0 なら整っています。
...
4. 関数の実行場所は `vercel.json` の `regions` で決めている（いまは `icn1` ＝ソウル）。
   **Supabase のプロジェクトと同じ地域**にすること。全ページがリクエストごとにデータベースを
   読むので、ここが離れていると往復のたびに待たされる。別の地域の Supabase を使うなら、ここを変える
   （以前は米国東部で動いていて、表示まで約3秒かかっていた）
...
名簿のアカウントは、何人もが同時に使う前提で扱う。ログアウトはその端末の
セッションだけを終える（ほかの人は入ったまま）。アカウントの削除と
パスワードの変更は断る（1人が変えると、ほかの人が入れなくなるため）。

来場者が書き込んだもの（予約・掲示・申し出など）は、そのまま残る。
触ってもらうための設定なので仕組みとしては正しいが、展示のあとは必要に応じて片づけること。
...
使い捨ての PostgreSQL を立て、マイグレーションを 0000 から順に流し、
`supabase/tests/*.sql` を実行します。本物の Supabase には触りません。

確かめているのは主に**「見えないはずのものが見えないこと」**です。
見えることはアプリを開けば分かりますが、見えないことは誰かが試さない限り
誰も気づきません。ポリシーを1行消したときに、ここが落ちてほしい。

pgTAP は使っていません。依存を増やすほど「手元では動くが CI では動かない」が
起きやすく、欲しいのは上の一点だけなので素の SQL で足ります。

| ファイル | 内容 |
| --- | --- |
| `_harness.sql` | 検査の道具。`test.as()` で誰として実行するかを切り替える |
| `_fixtures.sql` | 登場人物。大学2校・職員3人・学生3人・一般1人 |
| `01_rls.sql` | 可視範囲と、書き込みが RPC 以外に経路を持たないこと |
| `02_seals.sql` | 印影の焼き付けと、記録の書き換えに気づけるか |
| `03_handover.sql` | 代替わり。断れること、年度の数えかた |

> この仕組みを入れた初回に、実際の抜けが1件見つかりました。
> `0025` で `events.public_listed` を足したとき、アプリ側の問い合わせは
> 絞ったのに **RLS ポリシーを直していませんでした。** 匿名キーで REST を
> 直接叩けば、大学が学外に出さないと決めた行事まで読めていたことになります。
> `0031` で塞いでいます。
...
| `mobile.spec.ts` | 横にはみ出さないか。メニューの開閉、カードの横送り | — | ○ |
| `seal.spec.ts` | 印影の見本・保存・字数制限。学生には欄が出ないこと | ○ | — |
| `approval.spec.ts` | 承認のながれに印影が並ぶこと。申請書の印刷と、印刷時に消える操作 | ○ | — |
| `handover.spec.ts` | 代替わりの欄が畳まれていること。申し出と取り下げ | ○ | — |

### 狭い画面について

`Pixel 7`（412px）で回します。全部を流し直しはしません。印影の保存や
引き継ぎの申し出は、広い画面のぶんと同じデータを取り合ううえ、処理そのものは
幅で変わらないためです。狭い画面で確かめたいのは**配置と操作**なので、
読むだけの `access` と、狭い画面専用の `mobile` に絞っています。

いちばん効くのは**「本文が横にはみ出していないか」**で、11ページぶん見ています。
狭い画面の崩れはだいたいこれです。落ちたときは
**どの要素が原因かまで名前で出す**ようにしてあります（座標だけ言われても直せないので）。
自分で横スクロールする器（参加予定のカードなど）の中にあるものは、
はみ出していて正しいので除きます。
...
いまの件数は、データベース側が **72件**、画面側が **67件**（広い画面と狭い画面の合計）です。
...
app/
  ...
  staff/                 職員の対応待ち（ログイン後の着地点）
  staff/students/        職員による学生の登録
  not-found.tsx          見つからないときの画面（error.tsx は失敗したとき）
...
lib/
  ...
  handover.ts            代替わりと年度
  jst.ts                 日本時間の暦（日付と時刻は必ずここを通す）
  circle-categories.ts   サークルの分野（DB の CHECK 制約と同じ値）
...
supabase/
  migrations/            DB マイグレーション（0000〜0034、0012 は欠番）
  seed.sql               基本のテストデータ
  ...
  fix_demo_data.sql      デモデータを整える（seed の後に流す）
  promote_staff.sql      ロール付与（db:users の生成物）
  setup_all.sql          マイグレーション+seed の連結（生成物）
  reset_full.sql         【破壊的】作り直し用の初期化スクリプト
  tests/                 データベースの検証（db:test）
e2e/                     画面を通した検証（Playwright）
docs/
  poster.html            展示ポスター（A3×3枚。ブラウザから PDF に書き出せる）
vercel.json              関数の実行場所（Supabase と同じ地域にする）
scripts/
...
| `circle_handovers` | そのサークルのメンバー / その大学の職員 |
...
役職を変える RPC はもともとありましたが、それは「係を変える」操作であって
「代を継ぐ」ことではありません。代替わりには3つ要ります。

- **相手の承諾** … 断れない引き継ぎは引き継ぎではない。代表を一方的に
  押し付けられると困るので、承認と同じく相手が受けて初めて成立する
- **申し送り** … 次の代が最初に読む場所。口頭で消える情報を残す
- **年度** … いつの代かが分からないと、職員が「今年度まだ代替わりして
  いないサークル」を把握できない。年度は4月はじまりで数える

譲った人は退会させません。代表を降りたあとも在籍し続けるのが普通だからです。
抜けたいときは `leave_circle` を使います。そのときは管理者が新代表へ移って
いるので、「最後の管理者は抜けられない」にも引っかかりません。
...
紙の決裁では、誰が通したかは印影で分かります。氏名の文字列より、
押された跡そのものが記録になっている。同じものを持たせました。

職員ごとに「彫る文字（1〜4字）」と「形（丸・角）」を決め、押した時点の
印影を記録に焼き付けます。氏名のスナップショットと同じ考え方で、
あとから彫り直しても過去に押した跡は変わりません。
画像は持たず、文字と形から毎回描くので、紙に刷っても粗くなりません。

#### 記録が書き換えられていないか

承認の記録は、1件ずつ前の記録のハッシュを抱えています。途中の1行を
書き換えると、そこから後ろが合わなくなるので気づけます。

**公開鍵の署名ではありません。** 防ぎたいのは「後から静かに書き換えられる
こと」であって、なりすましの否認防止ではないためです。本気でやるなら
職員ごとの鍵が要りますが、それは鍵の配布と失効の話になり、大学の運用に
乗るかどうかから決める必要があります。

限界も明確です。**元の値にきっちり戻されると検知できません。**
分かるのは「いまの中身が押された当時と違うこと」だけで、途中で何が
あったかは追えません。この限界は `supabase/tests/02_seals.sql` に
テストとして書いてあります。

#### 申請書を紙の体裁で出す

大学の実務では、承認された申請は紙で保管されます。画面をそのまま印刷した
ものではなく、申請書として成立する紙が要ります。`/circles/[id]/print` が
それで、すりガラス・影・色面はすべて落とし、罫線と余白だけで組んでいます。
承認欄には押された印影が並び、空欄も残るので「あと何人か」が紙の上でも
読めます。

画面で開くと、同じ大学の職員には紙の上に「この申請を判断する」欄が出ます
（紙には刷られません）。申請書を読んだその場で押せて、押すと空の枠に
自分の印影が入ります。押したあとは、ほかの職員があと何人要るかを出します。

### 日付と時刻は、日本時間で扱う

サーバー（Vercel）は UTC で動きます。`new Date(年, 月, 日)`・`getDate()`・
時差の無い `"2026-09-24T13:00"` の読み込みは、どれも UTC として扱われます。
画面の日付や入力された時刻は日本時間なので、そのまま使うと**入力した 13:00 が 22:00 で
保存され、カレンダーでは 0時〜9時の予定が前日に載ります**（実際にそうなっていました）。

日付と時刻を扱うところは、必ず `lib/jst.ts` を通してください。

| 関数 | 使いどころ |
| --- | --- |
| `parseJstInput(date, time)` | フォームの日時を日本時間として読む |
| `toJstInput(date)` | `datetime-local` の初期値、`input[type=date]` の今日 |
| `jstParts(date)` / `jstDayKey(date)` | 年月日・曜日・時刻を取り出す、日付で束ねる |
| `jstDate(y, m, d)` | 日本時間の 0 時などにあたる瞬間を作る |

表示だけなら `Intl.DateTimeFormat` に `timeZone: "Asia/Tokyo"` を渡せば足ります。
日本には夏時間が無いので、時差は +9 時間の固定です。

### 明るさと、読める濃さ

明るさは `html[data-scheme]` で切り替えます。`prefers-color-scheme` を直接は見ません。
利用者の選択（`localStorage` の `uc-mode`）はサーバーから見えないので、
`app/layout.tsx` の小さなスクリプトが**描画の前に**属性を付けます。サーバーで決めると
一瞬ちがう色で描かれるためです。Tailwind の `dark:` も同じ属性を見るように
`@custom-variant` で差し替えてあります。

差し色は2つ持っています。

- `--accent` … ボタンや面の色
- `--accent-ink` … **文字に使う色**。明るい画面では一段濃く、暗い画面では一段明るい。
  `--accent` をそのまま文字にすると、暗い面の上で 2.2:1 まで落ちていた

小さいボタンは、見た目を変えずに押せる範囲だけを 44px 四方に広げています
（`app/globals.css` の `.tap-target` と、小さいボタンの類）。
