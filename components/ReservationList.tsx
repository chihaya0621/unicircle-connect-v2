...
export function ReservationList({
  ...
  if (reservations.length === 0) {
    return <p className="glass-empty">{emptyLabel}</p>;
  }

  const pendingCount = reservations.filter(
    (r) => r.status === "pending",
  ).length;
  // 2件以上溜まっているときだけ、まとめて承認の帯を出す
  const bulk = canDecide && pendingCount >= 2;

  return (
    <>
      {bulk && <BulkApproveBar count={pendingCount} />}
