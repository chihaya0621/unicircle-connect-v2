...
/** 1大学あたりの職員数。複数人承認を試せるよう2人以上にしている */
export const STAFF_PER_UNIVERSITY = 3;
...
export function buildDevUsers() {
  ...
  // 職員は大学あたり3人。サークルの設立・廃止は複数人の承認を
  // 求められる（0027）ので、1人しか居ないと複数承認を試せない。
  for (const key of STAFF_UNIVERSITY_KEYS) {
    const uni = UNIVERSITIES.find((x) => x.key === key);
    for (let i = 1; i <= STAFF_PER_UNIVERSITY; i++) {
      users.push({
        email: `staff${i}@${uni.key}.test`,
        name: `${uni.name.slice(0, 2)}職員${ORDINALS[i - 1]}`,
        role: "general",
        promoteTo: "staff",
        university_id: uni.id,
        universityName: uni.name,
